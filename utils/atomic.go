...
func (i *AtomicInt32) CompareAndSwap(olVal, newVal int32) (swapped bool) {
	return atomic.CompareAndSwapInt32(&i.int32, olVal, newVal)
}
...
// CompareAndSwap atomatically swaps the old with the new value.
func (d *AtomicDuration) CompareAndSwap(oldVal, neVal time.Duration) (swapped bool) {
	return atomic.CompareAndSwapInt64(&d.int64, int64(oldVal), int64(neVal))
}
...
// CompareAndSwap atomatically swaps the old with the new value.
func (s *AtomicString) CompareAndSwap(oldVal, newVal string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.str == oldVal {
		s.str = newVal
